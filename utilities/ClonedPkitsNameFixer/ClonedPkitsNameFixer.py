...
FETCH_KID_EXE = "/Users/cwallace/devel/Protiviti/ClonedPkitsNameFixer/FetchKeyId"
...
def main():
    ...
    parser.add_option("-d", "--originalPdtsFolder", dest="orig_pdts_folder", default="",
                      help="Folder containing End Entity Certs, pkcs12, smime and Trust Anchor Certs folders")
    parser.add_option("-e", "--clonedPdtsFolder", dest="cloned_pdts_folder", default="",
                      help="Folder containined cloned PDTS artifacts exported from PCP")
    parser.add_option("-f", "--renamedClonedPdtsFolder", dest="renamed_cloned_pdts_folder", default="",
                      help="Folder to receive renamed PDTS artifacts")
    parser.add_option("--alt_year", dest="alt_year", default="2017",
                      help="Value to use instead of 2011 in distinguished names")
    ...
    if options.orig_pkits_folder and options.cloned_pkits_folder and options.renamed_cloned_pkits_folder:
        ...
        for filename in orig_cert_files:
            ...
            issuer = issuer.replace("2011", options.alt_year)
        ...
        cloned_ee_files = glob2.glob(options.cloned_pkits_folder + '/EEs/*.der')
        cloned_ca_files = glob2.glob(options.cloned_pkits_folder + '/CAs/*.der')
        cloned_ta_files = glob2.glob(options.cloned_pkits_folder + '/TAs/*.der')
        cloned_cert_files = cloned_ca_files + cloned_ee_files + cloned_ta_files
    ...
    if options.orig_pdts_folder and options.cloned_pdts_folder and options.renamed_cloned_pdts_folder:
        orig_cert_files = glob2.glob(options.orig_pdts_folder + '/End Entity Certs/*.crt')
        orig_cert_files2 = glob2.glob(options.orig_pdts_folder + '/Trust Anchor Certs/*.crt')
        orig_cert_files = orig_cert_files + orig_cert_files2
        ...
        dest_folder = options.renamed_cloned_pdts_folder

        cloned_cert_files = glob2.glob(options.cloned_pdts_folder + '/Artifacts/*.fake.der')
        for filename in cloned_cert_files:
            openssl_command = OPENSSL_EXE + " x509 -inform DER -noout -serial -in '" + filename + "'"
            p = Popen(openssl_command, shell=True, stdout=PIPE)
            line = p.stdout.readline()
            ...
            composite = subject+issuer

            if composite in orig_dict.keys():
                orig_name = orig_dict[composite]
            else:
    ...
    if options.orig_pkits_folder and options.cloned_pkits_folder and options.renamed_cloned_pkits_folder:
        orig_crl_files = glob2.glob(options.orig_pkits_folder + '/crls/*.crl')

        orig_dict = {}
        for filename in orig_crl_files:
            openssl_command = OPENSSL_EXE + " crl -inform DER -noout -crlnumber -in '" + filename + "'"
            p = Popen(openssl_command, shell=True, stdout=PIPE)
            line = p.stdout.readline()
            subject = line.decode("utf-8").replace("subject= ", "").strip()

            openssl_command = OPENSSL_EXE + " crl -inform DER -noout -issuer -in '" + filename + "'"
            p = Popen(openssl_command, shell=True, stdout=PIPE)
            line = p.stdout.readline()
            issuer = line.decode("utf-8").replace("issuer= ", "").strip()
            issuer = issuer.replace("2011", options.alt_year)

            fetch_kid_command = FETCH_KID_EXE + " --idp " + filename
            p = Popen(fetch_kid_command, shell=True, stdout=PIPE)
            line = p.stdout.readline()
            idp = line.decode("utf-8").strip()
            idp = idp.replace("2011", options.alt_year)

            crlId = subject + issuer + idp
            if crlId in orig_dict.keys():
                print("ERROR: crlnumber+issuer: " + crlId + "; Old Filename: " + os.path.basename(filename))
                print("ERROR: crlnumber+issuer: " + crlId + "; New Filename: " + orig_dict[crlId])

            orig_dict[crlId] = os.path.basename(filename)

        dest_folder = options.renamed_cloned_pkits_folder

        dest_crl_folder = os.path.join(dest_folder, "crls")
        try:
            os.stat(dest_crl_folder)
        except:
            os.mkdir(dest_crl_folder)


        cloned_crl_files = glob2.glob(options.cloned_pkits_folder + '/CRLs/*.crl')
        for filename in cloned_crl_files:
            openssl_command = OPENSSL_EXE + " crl -inform DER -noout -crlnumber -in '" + filename + "'"
            p = Popen(openssl_command, shell=True, stdout=PIPE)
            line = p.stdout.readline()
            subject = line.decode("utf-8").replace("subject= ", "").strip()
            ...
            crlId = subject + issuer + idp
            if crlId not in orig_dict.keys():
                print("ERROR: did not find " + crlId)
                continue
            orig_name =  orig_dict[crlId]
            dest_file = os.path.join(dest_crl_folder, orig_name)
            dest_file = dest_file.replace(".resigned.crl", "")
            copyfile(filename, dest_file)
    ...
    # XXX***add parameter to identify fake CRLs folder then use code a la the below from BreakSig to break the
    #       signature on the busted signature test file
    # for filename in fake_crl_files:
    #     orig_name = os.path.basename(filename)
    #
    #     if orig_name in broken_crl_signatures:
